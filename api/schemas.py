...
class DateTimePaginationSchema(ma.Schema):
    ...
    def validate_schema(self, data, **kwargs):
        if data.get('offset') is not None and data.get('after') is not None:
            raise ValidationError('Não é permitido especificar ambos offset e after')
...
class UserSchema(ma.SQLAlchemySchema):
    ...
    questions_url = ma.URLFor('questions.user_all', values={'id': '<id>'},
                          dump_only=True)

    @validates('username')
    def validate_username(self, value):
        if not value[0].isalpha():
            raise ValidationError('Usuário deve começar com uma letra')
...
class QuestionSchema(ma.SQLAlchemySchema):
    ...
    body = ma.auto_field(required=True, validate=validate.Length(
        min=1, max=1024))
    answer = ma.auto_field(required=True, validate=validate.Length(
        min=1, max=512))    
